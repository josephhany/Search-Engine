...
int main(){
        ...
        init_var(keywords,"keywords.txt");
        init_var(impressions_init,"impressions.txt");
        init_var(clicks_init,"clicks.txt");
        vector<Edge> edges;
		...
		fill_file(graph,graph.impressions,"impressions.txt");
		fill_file(graph,graph.clicks,"clicks.txt");
...
void Graph::parse_query_or(string or_query,string dlim){
		...
		while (current_or != std::string::npos) {
			// sub_parsed_query.push_back(or_query.substr(previous_or, current_or-1 - previous_or));
...
void Graph::parse_query(string query,string dlim,string dlim2){
    ...
    // for(vector<string>ve:parsed_query)for(string v:ve)cout<<v<<endl;
}
